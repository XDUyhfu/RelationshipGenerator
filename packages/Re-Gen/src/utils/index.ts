...
const isObject = ( value: any ) => Object.prototype.toString.call( value ) === "[object Object]" && !isObservable( value ); // Observable 也是一个 Object 类型
const isPlainResult = ( result: ReturnResult ): result is PlainResult => ["number", "boolean", "string", "undefined"].includes( typeof result ) || isObject( result ) || Array.isArray( result ) || result === null;
...
export const handleObservable: () => ( source: AnyObservable ) => AnyObservable = () => ( source ) => new Observable( ( observer ) => {
	source.subscribe( {
		next: ( value ) => {
...
export const handlePromise: () => ( source: AnyObservable ) => AnyObservable = () => ( source ) => new Observable( ( observer ) => {
	source.subscribe( {
		next: ( value ) => {
...
export const handleUndefined: () => ( source: AnyObservable ) => AnyObservable = () => ( source ) => new Observable( ( observer ) => {
	source.subscribe( {
		next: ( value ) => {
...
export const handleCombine = ( type: CombineEnum, depends: AnyBehaviorSubject[] ): (( source: AnyObservable ) => AnyObservable) => ( source ) => type === CombineEnum.SELF ? source.pipe( withLatestFrom( ...depends ) ) : source.pipe( combineLatestWith( ...depends ) );
